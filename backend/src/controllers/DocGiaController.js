...
export default {
  ...
  async register(req, res) {
    ...
    if (
      lastDocGia &&
      lastDocGia.MaDocGia &&
      lastDocGia.MaDocGia.match(/^DG(\d+)$/)
    ) {
  ...
  async updateProfile(req, res) {
    ...
    const docGia = await DocGia.findByIdAndUpdate(req.user.id, updates, {
      new: true,
      runValidators: true,
    }).select("-password");
  ...
  async changePassword(req, res) {
    ...
    const isCurrentPasswordValid = await docGia.comparePassword(
      currentPassword
    );
  ...
  async getMyBorrows(req, res) {
    ...
    try {
      ...
      throw new AppError(
        "Không thể lấy danh sách sách đang mượn",
        500,
        "GET_BORROWS_ERROR"
      );
    }
  ...
  async getBorrowHistory(req, res) {
    ...
    try {
      ...
      throw new AppError(
        "Không thể lấy lịch sử mượn sách",
        500,
        "GET_HISTORY_ERROR"
      );
    }
