  ...
  return (req, _res, next) => {
    try {
      ...
      for (const [field, rules] of Object.entries(schema)) {
        ...
        console.log(
          `Validating field: ${field}, value: ${value}, required: ${rules.required}`
        );
        ...
        if (rules.type === "number") {
          ...
          if (rules.min !== undefined && num < rules.min) {
            errors.push(
              `${rules.label || field} phải lớn hơn hoặc bằng ${rules.min}`
            );
          }
          if (rules.max !== undefined && num > rules.max) {
            errors.push(
              `${rules.label || field} không được lớn hơn ${rules.max}`
            );
          }
        }
        ...
        if (typeof value === "string") {
          if (rules.minLength && value.trim().length < rules.minLength) {
            errors.push(
              `${rules.label || field} phải có ít nhất ${rules.minLength} ký tự`
            );
          }
          if (rules.maxLength && value.trim().length > rules.maxLength) {
            errors.push(
              `${rules.label || field} không được vượt quá ${
                rules.maxLength
              } ký tự`
            );
          }
        ...
        if (
          ...
          errors.push(
            rules.patternMessage ||
              `${rules.label || field} không đúng định dạng`
          );
        }

        if (rules.enum && !rules.enum.includes(value)) {
          errors.push(
            `${rules.label || field} phải là một trong: ${rules.enum.join(
              ", "
            )}`
          );
        }
...
export const validatePagination = (req, _res, next) => {
  try {
    ...
    if (isNaN(limitNum) || limitNum < 1 || limitNum > 100) {
      throw new AppError(
        "Số lượng bản ghi phải từ 1 đến 100",
        400,
        "VALIDATION_ERROR"
      );
    }
