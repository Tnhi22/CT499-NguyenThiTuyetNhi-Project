...
const theoDoiMuonSachSchema = new mongoose.Schema(
  {
    ...
    NhanVienMuon: {
      type: String,
      required: function () {
        return this.isActivate === 1; // Chỉ required khi phiếu đã được duyệt
      },
      ref: "NhanVien",
    },
    NhanVienTra: {
    ...
    PhiPhat: {
      ...
      validate: {
        validator: function (value) {
          // cho phép số nguyên, nếu bạn muốn cho phép số lẻ (vd: 2500.5) thì bỏ validate này
...
// Method: tính phí phạt từ chính document
theoDoiMuonSachSchema.methods.calculatePenaltyFee = function () {
  const overdueDays = this.SoNgayQuaHan;
  return overdueDays * 2000; // 2000 VND / ngày
};

// pre-save: mỗi lần save() sẽ auto update TrangThai + PhiPhat
  ...
  if (this.NgayTra) {
    ...
    if (this.NgayTra > this.NgayHenTra) {
      const overdueDays = Math.ceil(
        (this.NgayTra - this.NgayHenTra) / (1000 * 60 * 60 * 24)
      );
    ...
    const overdueDays = Math.ceil(
      (today - this.NgayHenTra) / (1000 * 60 * 60 * 24)
    );
    this.PhiPhat = overdueDays * 2000;
  } else {
...
// Method: trả sách (dùng được trực tiếp nếu muốn)
theoDoiMuonSachSchema.methods.returnBook = function (nhanVienTra, ghiChu) {
  ...
  return this.save(); // sẽ chạy pre('save') và tự tính PhiPhat
};
...
theoDoiMuonSachSchema.methods.extendDueDate = function (newDueDate, ghiChu) {
  ...
  if (ghiChu) this.GhiChu = ghiChu;
  return this.save(); // pre('save') cũng sẽ reset Trạng thái & PhiPhat hợp lý
};

// Static: update tất cả sách quá hạn (nếu muốn gọi từ controller)
theoDoiMuonSachSchema.statics.updateOverdueBooks = async function () {
  ...
  for (const book of overdueBooks) {
    const overdueDays = Math.ceil(
      (today - book.NgayHenTra) / (1000 * 60 * 60 * 24)
    );
