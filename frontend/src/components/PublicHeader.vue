          ...
          <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            <li class="nav-item">
              <router-link
                to="/"
                class="nav-link"
                :class="{ active: $route.name === 'PublicHome' }"
              >
            ...
            <li class="nav-item dropdown">
              ...
              >
                ...
                >
                  <router-link
                    :to="`/categories/${category.MaNhaXuatBan}`"
                    class="dropdown-item"
                    @click="closeDropdowns"
                  >
            ...
            <li class="nav-item" v-if="isAuthenticated">
              <a
                href="#"
                @click.prevent="showBorrowRegistrationModal"
                class="nav-link"
              >
            ...
            <form @submit.prevent="submitBorrowRegistration">
              ...
              <div class="mb-3">
                <label class="form-label"
                  >Chọn sách <span class="text-danger">*</span></label
                >
                <select
                ...
                >
                  ...
                  >
                    {{ book.TenSach }} (Còn:
                    {{ book.SoQuyenConLai || book.SoQuyen }})
                  </option>
                </select>
                <div v-if="borrowErrors.MaSach" class="invalid-feedback">
                  {{ borrowErrors.MaSach }}
                </div>
              </div>
              ...
              <div class="mb-3">
                ...
                <input
                  ...
                  :min="tomorrow"
                />
                <div v-if="borrowErrors.NgayHenTra" class="invalid-feedback">
                  {{ borrowErrors.NgayHenTra }}
                </div>
                <small class="form-text text-muted"
                  >Thời gian mượn tối đa 30 ngày</small
                >
              </div>
              ...
              <div class="alert alert-info">
                ...
                <strong>Lưu ý:</strong> Đây là đăng ký mượn sách. Yêu cầu của
                bạn sẽ được gửi đến thư viện để xem xét và phê duyệt.
              </div>
            </form>
          </div>
  ...
  setup() {
    ...
    const loadTopCategories = async () => {
      try {
        const response = await axios.get(
          "/nhaxuatban?limit=5&sortBy=TenNhaXuatBan"
        );
    ...
    const submitBorrowRegistration = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          showMessage(
            "Đăng ký mượn sách thành công! Vui lòng chờ thư viện phê duyệt.",
            "success"
          );
          closeBorrowModal();
        } else {
          showMessage(
            response.data.message || "Có lỗi xảy ra khi đăng ký",
            "error"
          );
        }
      } catch (error) {
