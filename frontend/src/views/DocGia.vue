                  ...
                  <tr v-for="docgia in docgiaList" :key="docgia.MaDocGia">
                    ...
                    <td>
                      <span
                        class="badge"
                        :class="
                          docgia.Phai === 'Nam' ? 'bg-info' : 'bg-warning'
                        "
                      >
                        {{ docgia.Phai }}
                      </span>
          ...
          <div class="modal-body">
            <form @submit.prevent="saveDocGia">
              <div class="row">
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="maDocGia" class="form-label"
                      >Mã độc giả <span class="text-danger">*</span></label
                    >
                    <input
                      type="text"
                      class="form-control"
                ...
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="phai" class="form-label"
                      >Phái <span class="text-danger">*</span></label
                    >
                    <select
                      class="form-select"
                      id="phai"
                ...
                <div class="col-md-6">
                  <div class="mb-3">
                    ...
                    <input
                      ...
                      placeholder="VD: docgia@email.com"
                    />
                ...
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="password" class="form-label"
                      >Mật khẩu
                      <span class="text-danger" v-if="!editingDocGia"
                        >*</span
                      ></label
                    >
                    <input
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>
              ...
              <strong
                >{{ deletingDocGia?.HoLot }} {{ deletingDocGia?.Ten }}</strong
              >?
            </p>
...
// Form data (Bổ sung Email và Password)
const formData = ref({
  MaDocGia: "",
  HoLot: "",
  Ten: "",
...
const editDocGia = (docgia) => {
  ...
  // Sao chép dữ liệu độc giả hiện tại vào formData, không bao gồm Password
  formData.value = { ...docgia, Password: "" };
  errors.value = {};
  showModal.value = true;
};
...
const validateForm = () => {
  ...
  if (!formData.value.NgaySinh) {
    ...
    // Điều kiện kiểm tra tuổi, giữ nguyên 5 đến 100
    if (age < 5 || age > 100) {
  ...
  // Password
  if (!editingDocGia.value && !formData.value.Password.trim()) {
    // Yêu cầu mật khẩu khi thêm mới
    errors.value.Password = "Mật khẩu là bắt buộc khi thêm mới";
  } else if (
    formData.value.Password.trim() &&
    formData.value.Password.length < 6
  ) {
    // Chỉ kiểm tra độ dài nếu có nhập (cả thêm mới và cập nhật)
...
const saveDocGia = async () => {
  ...
  saving.value = true;
  try {
    // BƯỚC QUAN TRỌNG: Chuẩn hóa tên trường Email và Password
    const payload = {
      // Lấy tất cả các trường khác
      MaDocGia: formData.value.MaDocGia,
      ...
      DienThoai: formData.value.DienThoai, // CHUẨN HÓA TÊN TRƯỜNG CHO BACKEND (phải là chữ thường)
      email: formData.value.Email,
      password: formData.value.Password,
    };

    if (editingDocGia.value) {
      // Nếu là chỉnh sửa và KHÔNG nhập mật khẩu mới, loại bỏ trường password (chữ thường)
      if (!payload.password) {
        delete payload.password;
      } // Update existing
      await api.put(`/docgia/${editingDocGia.value.MaDocGia}`, payload);
    } else {
      // Create new
      await api.post("/docgia", payload);
    }

    closeModal(); // Reload data after save
    loadDocGia();
    console.log("Độc giả đã được lưu thành công");
  } catch (error) {
    console.error("Error saving doc gia:", error); // Handle API errors
    if (error.response?.data?.errors) {
      // Nếu lỗi trả về là các trường chữ thường (như 'email'), bạn có thể cần
      // map lại nó sang chữ hoa để hiển thị trên form:
      const backendErrors = error.response.data.errors;
      const displayErrors = {};
      for (const key in backendErrors) {
        // Ví dụ: map 'email' (backend) thành 'Email' (frontend)
        const frontendKey = key.charAt(0).toUpperCase() + key.slice(1);
        if (formData.value.hasOwnProperty(frontendKey)) {
          displayErrors[frontendKey] = backendErrors[key];
        } else {
          displayErrors[key] = backendErrors[key];
        }
      }
      errors.value = displayErrors;
    } else if (error.response?.data?.message) {
      // Xử lý lỗi chung, ví dụ: Email/Mã độc giả bị trùng
...
const formatDate = (dateString) => {
  ...
  // Đảm bảo dữ liệu ngày tháng hợp lệ trước khi format
  try {
