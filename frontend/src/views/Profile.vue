              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <form @submit.prevent="submitEditProfile">
                    ...
                    <div class="row">
                      <div class="col-md-6 mb-3">
                        <label for="ngaySinhEdit" class="form-label"
                          >Ngày sinh</label
                        >
                        <input
                          type="date"
                    ...
                    <div class="row">
                      <div class="col-md-6 mb-3">
                        <label for="dienThoaiEdit" class="form-label"
                          >Điện thoại</label
                        >
                        <input
                          type="tel"
                    ...
                    <div class="mb-3">
                      <label for="avatarEdit" class="form-label"
                        >Avatar URL</label
                      >
                      <input
                        type="url"
                  ...
                  <table class="table table-hover">
                    ...
                    <tbody>
                      <tr v-for="history in borrowHistory" :key="history._id">
                        ...
                        <td>
                          {{
                            history.NgayTra ? formatDate(history.NgayTra) : "-"
                          }}
                        </td>
                        <td>
                          <span
                            class="badge"
                            :class="
                              history.NgayTra ? 'bg-success' : 'bg-warning'
                            "
                          >
...
export default {
  ...
  setup() {
    ...
    const loadingReturnId = ref(null);
    const message = ref("");
        ...
        return [
          {
            key: "HoLot",
            label: "Họ lót",
            value: user.value._doc.HoLot || "-",
          },
          { key: "Ten", label: "Tên", value: user.value._doc.Ten || "-" },
          {
            key: "NgaySinh",
            label: "Ngày sinh",
            value: user.value._doc.NgaySinh
              ? formatDate(user.value._doc.NgaySinh)
              : "-",
          },
          { key: "Phai", label: "Phái", value: user.value._doc.Phai || "-" },
          {
            key: "DiaChi",
            label: "Địa chỉ",
            value: user.value._doc.DiaChi || "-",
          },
          {
            key: "DienThoai",
            label: "Điện thoại",
            value: user.value._doc.DienThoai || "-",
          },
          { key: "email", label: "Email", value: user.value._doc.email || "-" },
          {
            key: "avatar",
            label: "Avatar",
            value: user.value._doc.avatar || "",
          },
        ];
      }
      return [];
    ...
    const initializeForm = () => {
      if (user.value?._doc) {
        ...
        profileForm.NgaySinh = user.value._doc.NgaySinh
          ? new Date(user.value._doc.NgaySinh).toISOString().split("T")[0]
          : "";
    ...
    const loadBorrowedBooks = async () => {
      try {
        ...
        const response = await axios.get(
          `/theodoimuonsach/docgia/${user.value._doc.MaDocGia}`
        );
        const allBorrows = response.data.data || [];
        console.log("All borrows from API:", allBorrows);
        borrowedBooks.value = allBorrows.filter((b) => !b.NgayTra);
        console.log("Borrowed (chưa trả):", borrowedBooks.value);
      } catch (error) {
        console.error("Lỗi loadBorrowedBooks:", error);
    ...
    const loadBorrowHistory = async () => {
      try {
        ...
        console.error("Lỗi loadBorrowHistory:", error);
    ...
    const handleReturnBook = async (borrow) => {
      if (!borrow || !borrow._id) {
        console.error("Borrow hoặc _id không hợp lệ:", borrow);
        showMessage("Không tìm thấy bản ghi mượn để trả!", "error");
        return;
      }

      console.log("Borrow khi bấm trả:", borrow);
      console.log("Call API: PUT /theodoimuonsach/" + borrow._id + "/tra");
      ...
      loadingReturnId.value = borrow._id;

      try {
        const response = await axios.put(`/theodoimuonsach/${borrow._id}/tra`, {
          NhanVienTra: user.value?._doc?.MaNhanVien || "NV001",
          GhiChu: "Trả sách từ trang độc giả",
        });

        if (response.data?.success) {
          showMessage("Trả sách thành công!", "success");
          await loadBorrowedBooks();
          await loadBorrowHistory();
        } else {
          showMessage(
            response.data?.message || "Không thể trả sách, vui lòng thử lại!",
            "error"
          );
        }
      } catch (error) {
