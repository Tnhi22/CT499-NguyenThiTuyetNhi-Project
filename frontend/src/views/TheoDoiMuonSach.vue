<template>
  <div class="container-fluid">
        ...
        <div class="card shadow">
          <div class="card-header py-3">
            ...
            <small class="text-muted">
              Nhân viên thao tác:
              <strong>{{
                currentUser?.HoTenNV || currentUser?.hoTenNV || "Không xác định"
              }}</strong>
              ({{ currentStaffId }})
            </small>
          </div>
              ...
              <div class="col-md-6">
                <div class="input-group">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    placeholder="Tìm kiếm theo mã, độc giả, sách..."
                    v-model="searchQuery"
                    @input="handleSearch"
                  />
                  <button
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <tr
                    ...
                    :key="record.MaTheoDoiMuonSach"
                  >
                    <td>
                      <span class="badge bg-primary">{{
                        record.MaTheoDoiMuonSach
                      }}</span>
                    </td>
                    <td class="fw-bold">{{ getReaderName(record) }}</td>
                    <td>{{ getBookTitle(record) }}</td>
                    <td>{{ formatDate(record.NgayMuon) }}</td>
                    <td :class="getDueDateClass(record)">
                    ...
                    <td>
                      <span
                        :class="'badge ' + getStatusClass(record.TrangThai)"
                      >
                        {{ record.TrangThai }}
                      </span>
                    </td>
    ...
    <div
      ...
      :style="{ display: showBorrowModalState ? 'block' : 'none' }"
      tabindex="-1"
    >
          ...
          <div class="modal-body">
            <form @submit.prevent="borrowBook">
              <div class="row">
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="docGia" class="form-label"
                      >Độc giả <span class="text-danger">*</span></label
                    >
                    <select
                      class="form-select"
                      id="docGia"
                    ...
                    <div class="invalid-feedback" v-if="borrowErrors.MaDocGia">
                      {{ borrowErrors.MaDocGia }}
                    </div>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="sach" class="form-label"
                      >Sách <span class="text-danger">*</span></label
                    >
                    <select
                      class="form-select"
                      id="sach"
                      v-model="borrowForm.MaSach"
                      :class="{ 'is-invalid': borrowErrors.MaSach }"
                    >
                      <option value="">Chọn sách</option>
                      <option
                        v-for="sach in availableBooks"
                        :key="sach.MaSach"
                        :value="sach.MaSach"
                      >
                        {{ sach.TenSach }} ({{ sach.MaSach }}) - Còn:
                        {{ sach.SoQuyenConLai || sach.SoQuyen }}
                      </option>
                    </select>
                    <div class="invalid-feedback" v-if="borrowErrors.MaSach">
                      {{ borrowErrors.MaSach }}
                    </div>
                  </div>
                </div>
                ...
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="ngayHenTra" class="form-label"
                      >Ngày hẹn trả <span class="text-danger">*</span></label
                    >
                    <input
                      type="date"
                      class="form-control"
                      ...
                      :class="{ 'is-invalid': borrowErrors.NgayHenTra }"
                      :min="tomorrow"
                    />
                    <div
                      class="invalid-feedback"
                      v-if="borrowErrors.NgayHenTra"
                    >
                      {{ borrowErrors.NgayHenTra }}
                    </div>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="nhanVienMuon" class="form-label"
                      >Nhân viên xử lý <span class="text-danger">*</span></label
                    >
                    <input
                      type="text"
                      class="form-control"
                      id="nhanVienMuon"
                      v-model="borrowForm.NhanVienMuon"
                      :class="{ 'is-invalid': borrowErrors.NhanVienMuon }"
                      placeholder="Mã nhân viên"
                    />
                    <div
                      class="invalid-feedback"
                      v-if="borrowErrors.NhanVienMuon"
                    >
                      {{ borrowErrors.NhanVienMuon }}
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div
      ...
      :style="{ display: showReturnModalState ? 'block' : 'none' }"
      tabindex="-1"
    >
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div v-if="returningRecord">
              <p>
                <strong>Độc giả:</strong> {{ getReaderName(returningRecord) }}
              </p>
              <p><strong>Sách:</strong> {{ getBookTitle(returningRecord) }}</p>
              <p>
                <strong>Ngày mượn:</strong>
                {{ formatDate(returningRecord.NgayMuon) }}
              </p>
              <p>
              ...
              <div class="mb-3">
                <label for="nhanVienTra" class="form-label"
                  >Nhân viên xử lý <span class="text-danger">*</span></label
                >
                <input
                  type="text"
                  class="form-control"
                  id="nhanVienTra"
                  v-model="returnForm.NhanVienTra"
                  :class="{ 'is-invalid': returnErrors.NhanVienTra }"
                  placeholder="Mã nhân viên"
                />
                <div class="invalid-feedback" v-if="returnErrors.NhanVienTra">
                  {{ returnErrors.NhanVienTra }}
                </div>
              </div>
              ...
              <div class="mb-3">
                ...
                <textarea
                  class="form-control"
                  id="ghiChuTra"
                  v-model="returnForm.GhiChu"
                  rows="3"
                  placeholder="Ghi chú về tình trạng sách khi trả (tùy chọn)"
                ></textarea>
              </div>
            </div>
          </div>
    ...
    <div
      ...
      :style="{ display: showExtendModalState ? 'block' : 'none' }"
      tabindex="-1"
    >
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div v-if="extendingRecord">
              <p>
                <strong>Độc giả:</strong> {{ getReaderName(extendingRecord) }}
              </p>
              <p><strong>Sách:</strong> {{ getBookTitle(extendingRecord) }}</p>
              <p>
                <strong>Ngày hẹn trả hiện tại:</strong>
                {{ formatDate(extendingRecord.NgayHenTra) }}
              </p>

              <div class="mb-3">
                <label for="ngayHenTraMoi" class="form-label"
                  >Ngày hẹn trả mới <span class="text-danger">*</span></label
                >
                <input
                  type="date"
                  class="form-control"
                  id="ngayHenTraMoi"
                  v-model="extendForm.NgayHenTra"
                  :class="{ 'is-invalid': extendErrors.NgayHenTra }"
                  :min="tomorrow"
                />
                <div class="invalid-feedback" v-if="extendErrors.NgayHenTra">
                  {{ extendErrors.NgayHenTra }}
                </div>
              </div>
              ...
              <div class="mb-3">
                ...
                <textarea
                  class="form-control"
                  id="ghiChuGiaHan"
                  v-model="extendForm.GhiChu"
                  rows="3"
                  placeholder="Lý do gia hạn (tùy chọn)"
                ></textarea>
              </div>
            </div>
          </div>
    ...
    <div
      ...
      :style="{ display: showDetailsModalState ? 'block' : 'none' }"
      tabindex="-1"
    >
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
                    ...
                    <div class="card-body">
                      <div class="mb-2">
                        ...
                        <span class="badge bg-primary ms-2">{{
                          viewingRecord.MaTheoDoiMuonSach
                        }}</span>
                      </div>
                      <div class="mb-2">
                        <strong>Ngày mượn:</strong>
                        <span class="ms-2">{{
                          formatDate(viewingRecord.NgayMuon)
                        }}</span>
                      </div>
                      <div class="mb-2">
                        <strong>Ngày hẹn trả:</strong>
                        <span
                          class="ms-2"
                          :class="getDueDateClass(viewingRecord)"
                        >
                      ...
                      <div class="mb-2">
                        ...
                        <span class="ms-2">{{
                          viewingRecord.NgayTra
                            ? formatDate(viewingRecord.NgayTra)
                            : "Chưa trả"
                        }}</span>
                      </div>
                      <div class="mb-2">
                        ...
                        <span
                          :class="
                            'badge ms-2 ' +
                            getStatusClass(viewingRecord.TrangThai)
                          "
                        >
                    ...
                    <div class="card-body">
                      <div class="mb-2">
                        ...
                        <span class="badge bg-success ms-2">{{
                          getDocGiaCode(viewingRecord)
                        }}</span>
                      </div>
                      <div class="mb-2">
                        <strong>Họ tên:</strong>
                        <span class="ms-2">{{
                          getReaderName(viewingRecord)
                        }}</span>
                      </div>
                      <div class="mb-2">
                        <strong>Điện thoại:</strong>
                        <span class="ms-2">{{
                          getDocGiaPhone(viewingRecord) || "Không có"
                        }}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="card-body">
                      <div class="mb-2">
                        ...
                        <span class="badge bg-info ms-2">{{
                          getSachCode(viewingRecord)
                        }}</span>
                      </div>
                      <div class="mb-2">
                        <strong>Tên sách:</strong>
                        <span class="ms-2">{{
                          getBookTitle(viewingRecord)
                        }}</span>
                      </div>
                      <div class="mb-2">
                        <strong>Tác giả:</strong>
                        <span class="ms-2">{{
                          getSachAuthor(viewingRecord) || "Không có"
                        }}</span>
                      </div>
                      <div class="mb-2">
                        <strong>Số quyển:</strong>
                        <span class="ms-2">{{
                          getSachQuantity(viewingRecord) || "Không có"
                        }}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="card border-warning mb-3">
                    ...
                    <div class="card-body">
                      <div class="mb-2">
                        ...
                        <span class="ms-2">{{
                          viewingRecord.NhanVienMuon || "Không có"
                        }}</span>
                      </div>
                      <div class="mb-2">
                        <strong>NV xử lý trả:</strong>
                        <span class="ms-2">{{
                          viewingRecord.NhanVienTra || "Chưa trả"
                        }}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="card border-secondary">
                    ...
                    <div class="card-body">
                      <p
                        class="mb-0"
                        :class="
                          viewingRecord.GhiChu ? '' : 'text-muted fst-italic'
                        "
                      >
          ...
          <div class="modal-footer">
            ...
            <div v-if="viewingRecord && viewingRecord.TrangThai !== 'Đã trả'">
              <button
                type="button"
                class="btn btn-success me-2"
                @click="showReturnModalFromDetails"
              >
                <i class="bi bi-check-circle me-2"></i>Trả sách
              </button>
              <button
                type="button"
                class="btn btn-warning"
                @click="showExtendModalFromDetails"
                v-if="viewingRecord.TrangThai === 'Đang mượn'"
              >
    ...
    <div
      ...
      v-if="
        showBorrowModalState ||
        showReturnModalState ||
        showExtendModalState ||
...
const filteredRecords = computed(() => {
  ...
  if (statusFilter.value) {
    filtered = filtered.filter(
      (record) => record.TrangThai === statusFilter.value
    );
  }
...
const visiblePages = computed(() => {
  const pages = [];
  const totalPagesComputed = Math.ceil(
    filteredRecords.value.length / itemsPerPage.value
  );
...
const availableBooks = computed(() => {
  return sachList.value.filter(
    (sach) => (sach.SoQuyenConLai || sach.SoQuyen) > 0
  );
});
...
const loadRecords = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      totalItems.value =
        response.data.pagination?.totalItems || recordsList.value.length;
...
const returnBook = async () => {
  ...
  try {
    await api.put(
      `/theodoimuonsach/${returningRecord.value.MaTheoDoiMuonSach}/tra`,
      returnForm.value
    );
    closeReturnModal();
    await loadRecords();
...
const extendDueDate = async () => {
  ...
  try {
    await api.put(
      `/theodoimuonsach/${extendingRecord.value.MaTheoDoiMuonSach}/giahan`,
      extendForm.value
    );
    closeExtendModal();
    await loadRecords();
...
${data.penaltyDetails
  .map(
    (item) =>
      `• ${item.book} - ${item.reader}: ${item.penalty.toLocaleString()} ₫ (${
        item.overdueDays
      } ngày)`
  )
  .join("\n")}
